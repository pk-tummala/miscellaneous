...
```bash
cd daily-job-status-automation
bash daily_job_status_report.sh
...
```bash
ONLY_JOB=Job_C bash daily_job_status_report.sh     # just one sequence
DEMO=1 bash daily_job_status_report.sh             # force demo
```
...
bash daily_job_status_report.sh --send
```
...
├── daily-job-status-automation-README.md         # this file
